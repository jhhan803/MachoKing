    ...
    public static GameObject[] Cut(GameObject victim, Vector3 anchorPoint, Vector3 normalDirection, Material capMaterial)
    {
      ...
      //  Destroy(victim.GetComponent<BoxCollider>());
       ...
       // GameObject leftSideObj = new GameObject("leftSide", typeof(MeshFilter), typeof(MeshRenderer));
        ...
        leftSideObj.GetComponent<MeshCollider>().sharedMesh = null;
        leftSideObj.GetComponent<MeshCollider>().sharedMesh = left_HalfMesh;

 
        rightSideObj.AddComponent<MeshCollider>().sharedMesh = right_HalfMesh;
        rightSideObj.AddComponent<Rigidbody>();
        rightSideObj.GetComponent<MeshCollider>().convex = true;
        rightSideObj.AddComponent<Cut>().capMaterial = capMaterial;
        rightSideObj.layer = 9;
        rightSideObj.transform.localScale = victim.transform.localScale;
        rightSideObj.AddComponent<OVRGrabbable>().SetgrabPoints(rightSideObj.GetComponent<MeshCollider>());
        rightSideObj.GetComponent<OVRGrabbable>().enabled = true;
        rightSideObj.GetComponent<Cut>().ingredient = leftSideObj.GetComponent<Cut>().ingredient;

        leftSideObj.transform.parent = GameObject.Find(leftSideObj.GetComponent<Cut>().ingredient).transform;
        rightSideObj.transform.parent = GameObject.Find(rightSideObj.GetComponent<Cut>().ingredient).transform;

        leftSideObj.tag = "Waittag";
        rightSideObj.tag = "Waittag";
